    ...
    <o-field id="test" :addons="false">
      ...
      >
        <o-table-column
          ...
          #default="{ row }"
        >
          <span v-if="column.field !== 'slot' && column.field !== 'partition'">{{
            row[column.field]
          }}</span>
          <o-button
            v-if="column.field == 'slot' && deviceStore.isABDevice"
            @click="setFlashSlot(data.findIndex((file) => file.filename == row.filename))"
            color="is-danger"
            >{{ row.slot }}</o-button
          >
          <o-input v-model="row.partition" v-if="column.field == 'partition'"></o-input>
          <o-button
            v-if="column.field == 'deleteButton'"
            @click="deleteDropFile(row.filename)"
            color="is-danger"
            >Delete</o-button
          >
          <ProgressBar
            v-if="column.field == 'size' && !row[column.field]"
      ...
      <o-upload :native="true" @update:modelValue="addFile($event)" multiple drag-drop>
        <section class="ex-center">
          <p>
            <o-icon icon="file-upload" size="large"> </o-icon>
          </p>
          <p>Drop your files</p>
        </section>
      </o-upload>
